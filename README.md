...
- Optional:
  ...
  - on FreeBSD, several Mason binary packages are skipped automatically; install `stylua`, `luacheck`, `shfmt`, `shellcheck`, `hadolint`, `lua-language-server`, and `clangd` from packages/ports if needed; `ruff` can be built by Mason when Rust/Cargo is installed
